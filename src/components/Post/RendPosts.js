...
export const RendPosts = () => {
  ...
  onGetPosts((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
      ...
      posts.innerHTML += `<div class="review-container">
                    ...
                    <div class="texticonspost">
                        <div class="inconDIV-like">
                            <img class="icon-like btn-like--solid" id="likeIcon${post.id}" data-id=${post.id} src=${likedByUser}"../img/heart-solid.svg"></img>
                            <img class="icon-like btn-like--empty" id="unLikeIcon${post.id}" data-id=${post.id} src=${likedByUser}"../img/corazon-vacio.png"></img>
                        </div>
      ...
      /* const btnLike = posts.querySelectorAll(`btn-like--empty`)
        btnLike.forEach((btn) =>{
            btn.addEventListener('click', async (e) => {
                const postId = e.target.dataset.id
                const full = posts.querySelector('btn-like--solid')
                //const empty = posts.querySelector('btn-like--empty')
                    try {
        ...
        const btnFull = posts.querySelector('btn-like--solid')
        btnFull.forEach((btn) =>{
            btn.addEventListener('click', async (e) => {
                const postId = e.target.dataset.id
                //const empty = posts.querySelector('btn-like--empty')
